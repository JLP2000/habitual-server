...
class Habit {
    ...
    static get all() {
        return new Promise (async (resolve, reject) => {
            try{
                let response = await db.query("SELECT * FROM habits;");
    ...
    static async findById(id) {
        return new Promise (async (resolve, reject) => {
           try { 
                let habitData = await db.query("SELECT * FROM habits WHERE habit_id = $1;", [id]);
    ...
    static async create(habitData) {
        return new Promise (async (resolve, reject) => {
            try {
                // console.log(habitData)
                const { name, start_date, interval_in_days, interval_in_months, end_date, note, colour, user_id} = habitData;
                const result = await db.query("INSERT INTO habits (name, start_date, interval_in_days, interval_in_months, end_date, note, colour, user_id) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *", [name, start_date, interval_in_days, interval_in_months, end_date, note, colour, user_id]);
                // console.log(result);
    ...
    static async update(habitData) {
        return new Promise (async (resolve, reject) => {
            try {
                ...
                const result = await db.query("UPDATE habits SET name = $1, note = $2, colour = $3 WHERE habit_id = $4 RETURNING *;", [name, note, colour, habit_id]);

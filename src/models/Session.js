...
class Session {
    ...
    static async create (user_id) {
        return new Promise (async (resolve, reject) => {
            try {
                ...
                let response = await db.query("INSERT INTO user_session (user_id, session_token) VALUES ($1, $2) RETURNING *;",
                                        [user_id, token]);
    ...
    static async findById(id) {
        return new Promise (async (resolve, reject) => {
            try {
                let response = await db.query("SELECT * FROM user_session WHERE session_id = $1", [id]);
    ...
    static async findBySessionToken(token) {
        return new Promise (async (resolve, reject) => {
            try {
                let response = await db.query("SELECT * FROM user_session WHERE session_token = $1", [token]);
                if (response.rows.length != 1) {
                    throw new Error("Unable to locate session.")
                }
                let session = new Session(response.rows[0]);
